...
import { apiConnectorPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [apiConnectorPlugin],
    });

    await request(server).get('/api/api-connector/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/api-connector/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/api-connector/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/api-connector/todos/${createdTodoItem.id}`)
